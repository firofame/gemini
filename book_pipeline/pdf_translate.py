...
def get_last_page_text(filename):
    ...
    parts = re.split(r'#*\s*Page\s+', content, flags=re.IGNORECASE)
    if len(parts) < 2:
        return ""
...
def process_single_batch(doc, batch_pages, prompt, model, previous_text="", max_retries=2):
    ...
    # Build per-batch instructions (the static prompt goes in system_instruction)
    page_list = " and ".join(str(p) for p in batch_pages)
    valid_headers = " and ".join(f"'## Page {p}'" for p in batch_pages)

    batch_instructions = (
        f"FORMATTING INSTRUCTIONS FOR THIS BATCH:\n"
    ...
    while attempts < max_retries:
        try:
            response = get_client().models.generate_content(
                model=model,
                contents=image_parts + [batch_instructions],
            ...
            if '429' in error_str or 'RESOURCE_EXHAUSTED' in error_str:
                ...
                wait = attempts * 10
            safe_print(f"  -> Error on Page(s) {page_list}: {e}. Retrying in {wait}s (attempt {attempts}/{max_retries})...")
            time.sleep(wait)
...
def sort_output(output_file):
    ...
    # Warn about duplicate page numbers
    page_nums = [p[0] for p in pages]
    seen = set()
    for pn in page_nums:
        if pn in seen:
            print(f"  ⚠️  Warning: duplicate page number {pn} in output file")
        seen.add(pn)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--prompt", default="prompt_ocr_translate_malayalam.txt",
        help="Prompt file (default: prompt_ocr_translate_malayalam.txt)"
    )
    parser.add_argument(
    ...
    output_file = args.output or args.pdf.replace(".pdf", ".md")
